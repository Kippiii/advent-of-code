...
let rec move_stack (n, i, j) stacks =
  if n = 0 then stacks else
    let final_stack_index = List.length stacks - 1 in
    let v = List.hd @@ List.nth stacks @@ (i-1) in
    let first_stack = push_stack final_stack_index v @@ pop_stack (i-1) @@ stacks in
    let next_stack = move_stack (n-1, i, j) first_stack in
    let final_stack = push_stack (j-1) v @@ pop_stack final_stack_index @@ next_stack in
    final_stack;;
...
let get_tops stacks = String.of_seq @@ List.to_seq @@ List.map List.hd @@ List.rev @@ List.tl @@ List.rev @@ stacks
