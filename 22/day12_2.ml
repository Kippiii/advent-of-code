...
let rec get_as_line line n =
  try
    let i = String.index_from line n 'a' in
    i::(get_as_line line (i+1))
  with _ -> [];;

let rec get_as lines length =
  match lines with
  | line::rest -> (get_as_line line 0) @ List.map (fun x -> x + length) (get_as rest length)
  | [] -> [];;
...
let bfs adj_list start ending =
  ...
  Queue.add (start, 0) q;
  let rec run_bfs () =
    if Queue.is_empty q then 999999999 else
...
let main () =
  ...
  let ass = get_as lines length in
  let all_starts = start::ass in
  let matrix = lists_to_matrix @@ parse_lines @@ lines in
  let adj_list = create_adj matrix in
  let all_ans = List.map (fun x -> bfs adj_list x ending) all_starts in
  print_int @@ List.hd @@ List.sort (-) @@ all_ans;;
