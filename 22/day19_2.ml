...
module State = struct
  ...
  let should_buy (s: state) (n: int) : bool =
    ...
    if n = 0 && (List.nth s.deltas 2) > 0 then false else
    if (n = 0 || n = 1) && (List.nth s.deltas 3) > 0 then false else
    if length_to_afford s n >= s.time - 1 then false else
    true
...
let get_max (bp: Blueprint.blueprint) : int =
  let max_geodes = State.max_geodes (State.init bp 32) in
  max_geodes;;
...
let main () =
  ...
  let maxes = List.map get_max blueprints in
  let ans = List.fold_left (fun x y -> x*y) 1 maxes in
  print_int ans;;
