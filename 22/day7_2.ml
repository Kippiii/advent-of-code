...
let rec find_best_folder (Folder arr) n =
  if List.length arr = 0 then 99999999999999999 else
  let rest = find_best_folder (Folder (List.tl arr)) n in
  match List.hd arr with
  | File n -> rest
  | f ->
    let cur_best = min rest (find_best_folder f n) in
    let folder_size = get_folder_size f in
    if folder_size < n then cur_best else min cur_best folder_size;;

let find_best f =
  let full_size = get_folder_size f in
  let open_space = 70000000 - full_size in
  if open_space >= 30000000 then 0 else find_best_folder f (30000000 - open_space);;

(* let main () = print_int @@ get_folder_sum @@ full_parse_commands @@ get_commands @@ ();;` *)
let main = print_int @@ find_best @@ full_parse_commands @@ get_commands @@ ();;
