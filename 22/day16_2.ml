module Graph = struct
  ...
  let explore matrix n flows =
    let memoh = Array.init n (fun _ -> Array.init 32 (fun _ -> Array.make (1 lsl n) (-1))) in
    let memoe = Array.init n (fun _ -> Array.init 32 (fun _ -> Array.make (1 lsl n) (-1))) in
    let rec run_explore index time mask is_elephant =
      if (1 lsl index) land mask <> 0 then (-999999999) else
      if is_elephant && memoe.(index).(time).(mask) <> -1 then memoe.(index).(time).(mask) else
      if (not is_elephant) && memoh.(index).(time).(mask) <> -1 then memoh.(index).(time).(mask) else
      let new_flows = List.init n (fun x ->
        if index = x then 0 else
        if matrix.(index).(x) + 1 > time then 0 else
        (flows.(x) * (time - matrix.(index).(x) - 1)) + run_explore x (time - matrix.(index).(x) - 1) (mask lor (1 lsl index)) is_elephant
      ) in
      let ans = List.fold_left max 0 new_flows in
      let new_ans = 
        if ans = 0 && not is_elephant then
          let new_mask = mask lor (1 lsl index) lxor 1 in
          max (run_explore 0 26 new_mask true) (flows.(0)*25 + run_explore 0 25 new_mask true)
        else
          ans
      in
      let _ = if is_elephant then memoe.(index).(time).(mask) <- new_ans else memoh.(index).(time).(mask) <- new_ans in
      new_ans
    in
    max (run_explore 0 26 0 false) (flows.(0)*25 + run_explore 0 25 0 false)
end;;
