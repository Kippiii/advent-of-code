...
let get_range ((sx, sy), (bx, by)) =
  let r = man_dist (sx, sy) (bx, by) in
  let y_dist = abs (2000000 - sy) in
  if y_dist > r then (-1, -2) else
  (sx - (r - y_dist), sx + (r - y_dist));;

let main () =
  let locs = get_input () in
  let range = List.fold_left Ranges.insert Ranges.empty (List.map get_range locs) in
  let ans = Ranges.get_length range in
  print_int ans;;
