...
let main () =
  ...
  let num_list = List.map (fun x -> 811589153 * x) (List.map int_of_string lines) in
  let spec_num_list = List.map2 (fun x y -> (x, y)) num_list (List.init (List.length lines) Fun.id) in
  let num_arr = Array.of_list spec_num_list in
  for i = 0 to 9 do
    ignore (List.map (update_arr num_arr) spec_num_list)
  done;
